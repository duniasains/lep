...
class NilaiWeb extends WebService {
    ...
    public function insertNoteToDB(){
        ...
        global $db;
        $q = "INSERT INTO ry_murid__name_nilai SET 
            
        name_nilai_judul = '$name_nilai_judul',
    ...
    public function guruNilai()
    {
        ...
        $guru->getKelasIDFromHomeroomID($ta,$guru->guru_id)->hr_kelas_id;
        
        $id = (isset($_GET['klsid'])?addslashes($_GET['klsid']):  $guru->getKelasIDFromHomeroomID($ta,$guru->guru_id)->hr_kelas_id);
        $kls = new Kelas();
        $kls->getByID($id);
       // 
        $mp_id = (isset($_GET['mp_id'])?addslashes($_GET['mp_id']): Gurumengajar::getMyLehrtMPID($guru->guru_id,$ta, $kls->kelas_id)->mj_mp_id);
        $mp = new Matapelajaran();
        $mp->getByID($mp_id);
