 ...
 */
 ...
 class Lang{
    ...
    public function loadLang(){
        ...
        try{
            if(!@include_once ($this->langPath.$filelang.".php")) throw new \Exception("Failed to include language file");
        }catch (\Exception $e) {
            //pr($e);
    ...
    public static function t($str,$return = 1){
        //disini bisa ditambah fungsi insert ke DB sehingga bs tau apa aja yang harus di replace !!
         ...
         if(isset($_lang[$str])){ 
             if($return) return $_lang[$str]; else echo $_lang[$str];
         }
         else{  
		$new = $str; // baru supaya mudah dicari variablenya
               // $new = ucwords(str_replace("_"," ",$str));
                 if($return) return "$new"; else echo "$new";                
         }
    }
